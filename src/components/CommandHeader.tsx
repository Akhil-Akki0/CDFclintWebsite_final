...
export const CommandHeader: React.FC<CommandHeaderProps> = ({
  ...
  return (
    <header className="h-10 bg-white border-b border-[#E2E8F0] px-3 flex items-center justify-between text-xs select-none shrink-0 z-20 shadow-[0_1px_2px_rgba(15,23,42,0.03)]">
      ...
      <div className="flex items-center gap-3">
        ...
        <div className="relative flex items-center">
          ...
          <div className="flex items-center gap-1.5 bg-[#F1F5F9] hover:bg-[#E2E8F0] transition-colors rounded px-2 py-1 text-[#0F172A] cursor-pointer border border-[#E2E8F0]">
            <Layers className="w-3.5 h-3.5 text-[#2563EB]" />
            <select
              id="scene-select"
              value={activePresetId}
              onChange={(e) => onPresetChange(e.target.value as AirfoilPresetId)}
              className="bg-transparent font-medium text-xs text-[#0F172A] focus:outline-none cursor-pointer pr-3"
            >
            ...
            <ChevronDown className="w-3 h-3 text-[#64748B] pointer-events-none -ml-3" />
          </div>
        </div>

        {/* Live Solver Status Pill */}
        <div className="flex items-center gap-1.5 px-2 py-0.5 rounded bg-[#F8FAFC] border border-[#E2E8F0] font-mono text-[11px]">
          ...
          <span className="font-semibold text-[#0F172A] uppercase tracking-wider text-[10px]">
            {simulationState.isRunning ? 'SOLVER ACTIVE' : 'SOLVER PAUSED'}
          </span>
